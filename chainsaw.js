...
window.addEventListener('load', function(){
  ...
  const DPR = Math.max(1, Math.min(2, Math.floor(devicePixelRatio || 1)));
  canvas.width = W * DPR;
  canvas.height = H * DPR;
  ...
  async function loadAtlases(){
    for(const k of Object.keys(ATLAS)){
      ATLAS[k].img = await loadImage(ATLAS[k].src);
    }
  }
  ...
  function drawAtlas(key, frame, x, y, flip=false){
    const a = ATLAS[key];
    if(!a || !a.img) return;
    const f = Math.floor(frame) % a.frames;
    const sx = f * a.fw;
    ctx.save();
    ctx.translate(x, y);
  ...
  let ATLASES_READY = false;
  loadAtlases().then(()=>{ ATLASES_READY = true; });
  ...
  for(let i=0;i<8;i++){
    bg.clouds.push({ x:Math.random()*W, y:30+Math.random()*140, s:60+Math.random()*120, v:0.1+Math.random()*0.2 });
  }
  ...
  function drawBG(dt){
    ...
    bg.clouds.forEach(c=>{
      ctx.beginPath();
      ctx.ellipse(c.x, c.y, c.s, c.s*0.35, 0, 0, Math.PI*2);
      ctx.fill();
      c.x -= c.v * dt / 16;
    ...
    function tree(x, base, h, color){
      ...
      ctx.beginPath();
      ctx.moveTo(x, base);
      ctx.lineTo(x-18, base-h*0.6);
      ctx.lineTo(x+18, base-h*0.6);
      ctx.closePath();
      ctx.fill();
      ctx.fillRect(x-4, base-h*0.6, 8, h*0.6);
    }
    ...
    for(let i=bg.leaves.length-1;i>=0;i--){
      const L = bg.leaves[i];
      ctx.fillStyle = ['#f39a2e','#e6c04a','#c34b3b'][i%3];
      ctx.fillRect(L.x, L.y, 3, 3);
      L.x += L.vx; L.y += L.vy;
      if((L.life -= dt) <= 0 || L.x < -10) bg.leaves.splice(i,1);
    }
  ...
  function melee(){
    const p = st.p;
    if(p.cool>0 || p.state==='melee') return;
    p.state='melee'; p.anim=0; p.cool=280;
    play('chainsaw_swing', 0.9);
  }

  function throwAxe(){
    const p = st.p;
    if(p.state==='throw') return;
    p.state='throw'; p.anim=0;
    st.projectiles.push({ x:p.x+p.w/2, y:p.y+p.h/2, vx:(p.face>0?7:-7), vy:-0.2, w:22, h:18, rot:0, dmg:40 });
    play('axe_throw', 0.8);
  }

  addEventListener('keydown', e=>{
    if(e.key === ' ') melee();
    if(e.key.toLowerCase() === 'e') throwAxe();
  });

  // SFX
  const SFX = {};
  ['chainsaw_swing','axe_throw','axe_hit','tornado_whoosh','aoe_burst','bolt_fire','boss_crit'].forEach(n=>{
    const a=new Audio('assets/'+n+'.wav');
    a.preload='auto';
    SFX[n]=a;
  });
  function play(n, vol=1){
    try{
      const a = SFX[n].cloneNode();
      a.volume = vol;
      a.play();
    }catch(_){}
  }
  ...
  // Spawns
  function spawnMini(){
    const e={ type:'mini', x:W-140, y:110, w:128, h:128, hp:420, max:420, touch:20, atk:0, tele:0 };
    st.enemies.push(e);
  }
  function spawnBoss(){
    const e={ type:'hurricane', x:W-220, y:40, w:192, h:192, hp:900, max:900, touch:28, atk:0, tele:0, f:0 };
    st.enemies.push(e);
  }
  ...
  function update(dt){
    ...
    st.p.x += vx * st.p.spd * dt/16;
    st.p.y += vy * st.p.spd * dt/16;
    if(vx!==0) st.p.face = vx>0 ? 1 : -1;

    st.p.x = Math.max(16, Math.min(W-16-st.p.w, st.p.x));
    st.p.y = Math.max(80, Math.min(H-80-st.p.h, st.p.y));
    st.p.cool = Math.max(0, st.p.cool-dt);
    st.p.anim += dt;
    ...
    if(st.t > st.nextSpawn){
      st.nextSpawn = st.t + Math.max(300, st.spawnRate - st.wave*40) + Math.random()*400;
      const t = Math.random()<0.6 ? 'tornado' : 'zig';
      const y = 160 + Math.random()*(H-260);
    ...
    for(let i=st.enemies.length-1;i>=0;i--){
      ...
      if(e.type==='tornado' || e.type==='zig'){
        ...
        if(e.type==='zig'){
          e.y += (e.vy || 1.2)*dt/16;
          if(e.y<80 || e.y>H-120) e.vy = -e.vy;
        }
        e.f = (e.f||0) + dt*0.02;
      }
      else if(e.type === 'mini'){
        if(!st.boss){
          st.boss = true; st.bossEntity = e;
          bossbar.style.display='block';
          bosslabel.textContent='Elemental Storm Titan';
        }
        e.x += (-1.0)*dt/16;
        e.atk = (e.atk||0) + dt;
        ...
        if(!st.boss){
          st.boss = true; st.bossEntity = e;
          bossbar.style.display='block';
          bosslabel.textContent='Hurricane Lord';
          bgmMain.pause();
          bgmBoss.currentTime=0; bgmBoss.volume=0.8; bgmBoss.play().catch(()=>{});
        }
        e.x += (-0.8)*dt/16;
        e.f = (e.f||0) + dt*0.02;
        e.atk = (e.atk||0) + dt;
      ...
      if(e.hp !== undefined && e.hp <= 0){
        ...
        if(e.type==='mini' || e.type==='hurricane'){
          st.boss=false; st.bossEntity=null;
          bossbar.style.display='none';
          bgmBoss.pause(); try{ bgmMain.play(); }catch(_){}
        }
      } else if(e.x < -240){
        st.enemies.splice(i,1);
      }
    ...
    for(let i=st.bolts.length-1;i>=0;i--){
      ...
      if(b.x<0 || b.x>W || b.y<60 || b.y>H-60 || b.life<=0){
        st.aoe.push({ x:b.x, y:b.y, r:36, life:350 });
        st.bolts.splice(i,1);
        play('aoe_burst', 0.6);
        continue;
      }
    ...
    // AoE fadeout
    for(let i=st.aoe.length-1;i>=0;i--){
      const a = st.aoe[i];
      a.life -= dt;
      if(a.life<=0) st.aoe.splice(i,1);
    }

    // Axe projectiles
    for(let i=st.projectiles.length-1;i>=0;i--){
      const a = st.projectiles[i];
      a.x += a.vx*dt/16;
      a.y += a.vy*dt/16;
      a.vy += 0.02;
    ...
    if(st.boss && st.bossEntity){
      const e = st.bossEntity;
      bossbar.style.display='block';
      bosslabel.textContent = (e.type==='mini') ? 'Elemental Storm Titan' : 'Hurricane Lord';
      const pct = Math.max(0, e.hp/e.max);
      bossfill.style.width = (pct*100) + '%';
      bossfill.style.filter = (pct<0.25 && Math.floor(st.t/120)%2===0) ? 'brightness(1.4)' : 'none';
    } else {
      bossbar.style.display='none';
    }
  }
  ...
  function render(){
    ...
    // HP bar
    ctx.fillStyle='#122032'; ctx.fillRect(16,16,240,12);
    ctx.fillStyle='#2e95ff'; ctx.fillRect(16,16,240*(st.p.hp/st.p.max),12);
    ctx.strokeStyle='#2a3b52'; ctx.strokeRect(16,16,240,12);
  }
  ...
  function loop(ts){
    const dt = ts - last;
    last = ts;
    if(canvas.style.display!=='none' && ATLASES_READY){
      update(dt);
      render();
    }
  ...
  startBtn.onclick = ()=>{
    ...
    bossbar.style.display='none';
    bgmBoss.pause();
    bgmMain.currentTime=0; bgmMain.volume=0.6;
    bgmMain.play().catch(()=>{});
  };
});
