...
// Robust start handler with fallbacks
function __startGameOnce(){
  const menu = document.getElementById('menu');
  const canvasEl = document.getElementById('game');
  if(menu) menu.style.display='none';
  if(canvasEl){ canvasEl.style.display='block'; canvasEl.focus && canvasEl.focus(); }

  // Reset core state if defined
  try {
    if (typeof st !== 'undefined') {
      st.over=false; st.paused=false; st.t=0; st.enemies.length=0;
      st.wave=0; st.nextSpawn=0;
      if (st.p){ st.p.x = 80; st.p.y = 360; st.p.state='idle'; st.p.anim=0; st.p.cool=0; st.face=1; }
    }
  } catch(e){ console.warn('State reset skipped', e); }

  // Kick BGM (gracefully handle autoplay policy)
  try{ bgmBoss.pause(); bgmBoss.currentTime=0; }catch{}
  try{ bgmMain.volume=0.6; bgmMain.currentTime=0; bgmMain.play().catch(()=>{}); }catch{}

  // Guard against multiple starts
  window.__gameStarted = true;
}
function __bindStartControls(){
  const btn = document.getElementById('startBtn');
  const menu = document.getElementById('menu');
  const startWrapper = (ev)=>{
    if(window.__gameStarted) return;
    ev && ev.preventDefault && ev.preventDefault();
    __startGameOnce();
  };
  if(btn){
    btn.addEventListener('click', startWrapper, { once:true });
    btn.addEventListener('touchend', startWrapper, { once:true, passive:false });
    // Ensure pointer events on the button
    btn.style.pointerEvents = 'auto';
  }
  if(menu){
    // Clicking the menu background also starts
    menu.addEventListener('click', (e)=>{
      if(window.__gameStarted) return;
      if(e.target && e.target.id !== 'startBtn'){ __startGameOnce(); }
    }, { once:true });
  }
  // Keyboard fallback
  const keyStart = (e)=>{
  ...
  // Audio unlock on first gesture (Chrome/Safari)
  const unlock = ()=>{
    try{
      [bgmMain, bgmBoss].forEach(a=>{
        try{ a.muted=false; a.play().then(()=>a.pause()).catch(()=>{}); }catch{}
      });
    }catch{}
...
// Bind after current task to ensure DOM is ready
setTimeout(__bindStartControls, 0);
