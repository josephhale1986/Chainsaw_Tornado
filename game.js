...
let DEBUG = false;
const dbg = { lines: [] };
window.addEventListener('keydown', e=>{ if(e.key==='F2' || e.key==='f2'){ DEBUG=!DEBUG; }});

function drawDebugPanel(){
  if(!DEBUG) return;
  const pad=8, w=360, h=180;
  ctx.save();
  ctx.globalAlpha=0.92;
  ctx.fillStyle='rgba(0,0,0,0.75)';
  ctx.fillRect( W - w - pad, pad, w, h );
  ctx.strokeStyle='#2a3b52'; ctx.strokeRect( W - w - pad, pad, w, h );
  ctx.fillStyle='#9fc1ff'; ctx.font='bold 14px system-ui';
  ctx.fillText('DEBUG (F2 to toggle)', W - w, pad+20);
  ctx.fillStyle='#cde2ff'; ctx.font='12px system-ui';
  const startY = pad+40;
  const lines = [
    'Atlases:',
    'hero: ' + (atlases.hero ? (atlases.hero.fw+'x'+atlases.hero.fh+' | natural='+ (atlases.hero.img ? atlases.hero.img.naturalWidth+'x'+atlases.hero.img.naturalHeight : 'n/a') +' | frames='+atlases.hero.frames +' | ok='+atlases.hero.ok) : 'loading'),
    'min:  ' + (atlases.min ? (atlases.min.fw+'x'+atlases.min.fh+' | natural='+ (atlases.min.img ? atlases.min.img.naturalWidth+'x'+atlases.min.img.naturalHeight : 'n/a') +' | frames='+atlases.min.frames +' | ok='+atlases.min.ok) : 'loading'),
    'zig:  ' + (atlases.zig ? (atlases.zig.fw+'x'+atlases.zig.fh+' | natural='+ (atlases.zig.img ? atlases.zig.img.naturalWidth+'x'+atlases.zig.img.naturalHeight : 'n/a') +' | frames='+atlases.zig.frames +' | ok='+atlases.zig.ok) : 'loading'),
    'titan:'+ (atlases.titan ? (atlases.titan.fw+'x'+atlases.titan.fh+' | natural='+ (atlases.titan.img ? atlases.titan.img.naturalWidth+'x'+atlases.titan.img.naturalHeight : 'n/a') +' | frames='+atlases.titan.frames +' | ok='+atlases.titan.ok) : 'loading'),
    'hurr: ' + (atlases.hurr ? (atlases.hurr.fw+'x'+atlases.hurr.fh+' | natural='+ (atlases.hurr.img ? atlases.hurr.img.naturalWidth+'x'+atlases.hurr.img.naturalHeight : 'n/a') +' | frames='+atlases.hurr.frames +' | ok='+atlases.hurr.ok) : 'loading'),
    'State: wave='+st.wave+' boss='+(st.boss? 'yes':'no')
  ];
  let y = startY;
  for(const ln of lines){ ctx.fillText(ln, W - w + 10, y); y += 16; }
  ctx.restore();

  // Optional: draw frame grid for hero when atlas detected not-ok
  if(atlases.hero && !atlases.hero.ok && atlases.hero.img){
    ctx.save();
    ctx.globalAlpha=0.25;
    ctx.strokeStyle='#00ff55';
    const fw = atlases.hero.fw, fh = atlases.hero.fh;
    for(let i=0;i<15;i++){
      const gx = st.p.x + i*2; // small stacked grid next to player
      ctx.strokeRect(gx, 60, fw, fh);
    }
    ctx.restore();
  }
}
